...
(function ($) {
    ...
    /*-----------------------------------------
     6. Minicart Dropdown and Search Bar-------
     ------------------------------------------ */
    async function headerBarLoad() {

        const resp = await fetch("header.html");
        const headerContent = await resp.text();
        document.getElementById("pageWrapper").insertAdjacentHTML("afterbegin", headerContent);
        window.scrollTo(0, 0);

        loadCartCount();

        loadSessionDetailsInHeader();
...
//            dots: false,
//            infinite: true,
//            slidesToShow: 5, // Use slidesToShow instead of item
//            slidesToScroll: 1,
//            responsive: [
//                {
//                    breakpoint: 1024,
...
var sampleProductImageList = [
    "assets/images/autoparts/bodyPartsAndMirrors.jpg",
    "assets/images/autoparts/brakesSuspensionAndSteering.jpg",
    "assets/images/autoparts/engineAndDrivetrain.jpg",
    "assets/images/autoparts/exhaustSystem.jpg",
    "assets/images/autoparts/exteriorAccessories.jpg",
    "assets/images/autoparts/headlightsAndLighting.jpg",
    "assets/images/autoparts/interiorAccessories.jpg",
    "assets/images/autoparts/toolsAndGarage.jpg"
];

async function loadFooter() {
    const resp = await fetch("footer.html");
    const footerContent = await resp.text();
    document.getElementById("footer-content-marker").insertAdjacentHTML("beforebegin", footerContent);
}
loadFooter();

async function loadSessionDetailsInHeader() {

    const response = await fetch("LoadHeaderDetails");
    ...
    if (response.ok) {
        ...
        if (json.success) {
            ...
            document.getElementById("nameAnchor").innerHTML = jsonContent.first_name + " " + jsonContent.last_name;
            document.getElementById("nameAnchor").classList.remove("d-none");
            document.querySelectorAll(".signOutMenu").forEach(element=>{
                element.classList.remove("hide");
            });
            document.getElementById("createAccountAnchor").classList.add("d-none");
            document.getElementById("loginAnchor").classList.add("d-none");
        } else {

            if (json.content !== "NotLoggedIn") {
                popup.error({
...
async function addToCart(id, qty) {
    ...
    const response = await fetch(
            "AddToCart?id=" + id + "&qty=" + qty + "&operation=plus"
            );
...
async function loadCartCount() {
    const response = await fetch("LoadCartCount");
