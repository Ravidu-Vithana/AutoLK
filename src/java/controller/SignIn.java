...
public class SignIn extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (user_DTO.getEmail().isEmpty()) {
            ...
            response_DTO.setContent("Please enter the password");
        } else {
            ...
            if (!criteria1.list().isEmpty()) {
                            ...
                            for (Cart_DTO cart_DTO : sessionCart) {
                                ...
                                for (Cart cart : dbCart) {
                                    if (cart_DTO.getProduct().getId() == cart.getProduct().getId()) {
                                        //same product in session Cart & Db Cart
                                        isFoundInCart = true;
                                        
                                        if ((cart_DTO.getQty() + cart.getQty()) <= cart.getProduct().getQty()) {
                                            //updated quantity is available
                                            cart.setQty(cart_DTO.getQty() + cart.getQty());
                                            session.update(cart);
                                        } else {
                                            ///updated quantity is not available. Setting max quantity
                ...
                //invalid credentials
                response_DTO.setContent("Invalid credentials.");
            }
