                        ...
                        if (request.getSession().getAttribute("user") != null) {
                            ...
                            if (criteria2.list().isEmpty()) {
                                ...
                                if (productQty <= product.getQty()) {
                                    ...
                                    response_DTO.setContent("Product added to Cart successfully!");

                                } else {
                                    //quantity not available
                                    response_DTO.setContent("Add to Cart failed. Requested quantity is not available");
                                }

                            } else {
                                //item already found in cart
                                Cart cartItem = (Cart) criteria2.uniqueResult();
                                if (operation.equals("plus") && (cartItem.getQty() + productQty) <= product.getQty()) {
                                ...
                                } else if (operation.equals("minus") && (cartItem.getQty() - productQty) > 0) {
                                    cartItem.setQty(cartItem.getQty() - productQty);
                                    transaction.commit();
                                    ...
                                    response_DTO.setContent("Cart update failed.");
                                }
                            ...
                            if (httpSession.getAttribute("sessionCart") != null) {
                                ...
                                if (foundCart_DTO != null) {
                                    ...
                                    if (operation.equals("plus") && (foundCart_DTO.getQty() + productQty) <= product.getQty()) {
                                        //update quantity
                                        foundCart_DTO.setQty(foundCart_DTO.getQty() + productQty);
                                        response_DTO.setSuccess(true);
                                        response_DTO.setContent("Product added to Cart successfully!");
                                    } else if (operation.equals("minus") && (foundCart_DTO.getQty() - productQty) > 0) {
                                        //update quantity
                                        foundCart_DTO.setQty(foundCart_DTO.getQty() - productQty);
                                        response_DTO.setSuccess(true);
                                        response_DTO.setContent("Product added to Cart successfully!");
                                    ...
                                    if (productQty <= product.getQty()) {
                                        ...
                                        //quantity not available
                                        response_DTO.setContent("Cart update failed. Requested quantity is not available");
                                    }
